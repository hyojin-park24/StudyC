/* 06 반복문 
	...
	- 반복문 : 일정 조건을 만족하는 동안 같은 실행문을 반복
				...
				- 반복문 이미지 필요 (p.154)
	[반복문 종류 세가지]
	- while(조건식) : 반복 문장을 실행하기 전에 반복 조건을 먼저 검사한다
	  {
			실행문;
	  }
	  1. 조건을 물어보고 만족하는 동안
	...
	- for(초기식; 조건식; 증감식) : 반복 횟수가 정해진 경우 사용
	  {
		실행문;
	  }
	  1. 초기값을 최초 설정한 다음
...
int main()
{
	...
	{
		a = a * 2;					//a의 값을 2배로 늘린다.
	} while (a < 10);				//a가 10보다 작으면 75행을 반복

	printf("a : %d\n", a);			//반복이 끝난 후 a값 출력

	//do~while문 특징 : 조건식과 관계없이 반복할 문장을 최소 한 번은 실행함
